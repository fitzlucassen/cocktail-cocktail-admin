...
class Upload extends Helper
{
	...
	public function file($file)
	{
		...
		//set tmp path
		$this->tmp_name = $this->file_post['tmp_name'];
	}
	 ...
	 * @return bool
	 */
	protected function check_file_array()
	{
		$isOk = isset($this->file_post['error'])
			&& !empty($this->file_post['name'])
			&& !empty($this->file_post['type'])
			&& !empty($this->file_post['tmp_name'])
			&& !empty($this->file_post['size']);

		if ($isOk) {
			if (is_array($this->file_post['name']))
				$this->file_post['name'] = $this->file_post['name'][0];
			if (is_array($this->file_post['tmp_name']))
				$this->file_post['tmp_name'] = $this->file_post['tmp_name'][0];
			if (is_array($this->file_post['size']))
				$this->file_post['size'] = $this->file_post['size'][0];
			if (is_array($this->file_post['type']))
				$this->file_post['type'] = $this->file_post['type'][0];
		}
	...
	{
		return \finfo::file($this->tmp_name, FILEINFO_MIME_TYPE);
	}
